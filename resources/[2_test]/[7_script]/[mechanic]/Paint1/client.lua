...
Citizen.CreateThread(function()
	...
	ped = CreatePed("PED_TYPE_CIVFEMALE", "a_m_y_hasjew_01", -330.9734, -115.8132, 37.9913, 62.9582, false, true)
	SetBlockingOfNonTemporaryEvents(ped, true)
	FreezeEntityPosition(ped, true)
	end)

RegisterNetEvent('ledjo_paint:client:setLocationBusy', function(pos, value)
    isBusy[pos] = value
end)

RegisterNetEvent('ledjo_paint:client:initalizePaint', function(id, vehicle, color, isPrimary)
...
RegisterNetEvent('ledjo_paint:client:stopPaint', function(id)
    if #(GetEntityCoords(PlayerPedId()) - Config.Locations[id].control) <= 15.0 then
        isSpraying = false
    end
end)
...
InitializePaint = function(pos)
    ...
    TriggerServerEvent('ledjo_paint:server:setLocationBusy', pos, true)
    ...
    if not input then
        TriggerServerEvent('ledjo_paint:server:setLocationBusy', pos, false)
        return
    end
    ...
    TriggerServerEvent('ledjo_paint:server:initalizePaint', pos, VehToNet(vehicle), Hex2Rgb(input[3]), isPrimary)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerServerEvent('ledjo_paint:server:setLocationBusy', pos, false)
        TriggerServerEvent('ledjo_paint:server:stopPaint', pos)
    end)
end
