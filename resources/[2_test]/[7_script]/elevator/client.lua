...
local function enterElevator(zone)
    ...
    for i = 1, #zone do
        for k,v in pairs(zone[i]) do
            options[k] = {value = k, label = ('Etage %s'):format(k)}
        end
    end

    local input = lib.inputDialog('Menu Ascenseur', {
        { type = 'select', label = "Selectionner l'étage", options = options}
    })
                ...
                if ESX.PlayerData.job.name == jobName then
                    ...
                    if self.isClosest and self.currentDistance < 1.2 then
                        if not hasTextUi then
                            hasTextUi = true
                            lib.showTextUI(('[E] - Entrer %s'):format(cfg.Elevators[self.eleId].name))
                        end
