...
-- safe trim (évite les plantages si nil)
if not string.strtrim then
    function string.strtrim(s)
...
MySQL.insert('INSERT INTO `uniq_garage` (owner, name, data) VALUES (?, ?, ?)', {
    identifier, garage, json.encode(Garages[identifier][garage])
})
...
lib.callback.register('uniq_garage:cb:BuyCustomization', function(source, data)
    ...
    -- s'assure que la table existe pour cet étage
    playerGarage.style = playerGarage.style or {}
    playerGarage.style[data.floor] = type(playerGarage.style[data.floor]) == 'table' and playerGarage.style[data.floor] or {}
    local floorStyles = playerGarage.style[data.floor]
    
    for _, style in ipairs(floorStyles) do
        if data.interior.color then
            -- compare les couleurs seulement si les 2 existent
            local sc = tonumber(style.color)
            local dc = tonumber(data.color)
            if style.name == data.interior.name and sc and dc and sc == dc then
...
local function saveToDB()
    ...
    for owner, garages in pairs(Garages) do
        for name, garageData in pairs(garages) do
            ...
            insertTable[#insertTable + 1] = { query = query, values = {owner, name } }
        end
    end
