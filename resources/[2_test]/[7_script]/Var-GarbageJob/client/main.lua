...
Citizen.CreateThread(function()
    while true do
        ...
        if InService then
            Wait = 0
            local pPed, pCoords = PlayerPedId(), GetEntityCoords(PlayerPedId(), false)
            ...
            if not InDutty then
                if not Route then
                    ...
                    -- 🔹 Application tenue Illenium (homme/femme)
                    if not ClothesChanged then
                        if GetEntityModel(pPed) == GetHashKey("mp_m_freemode_01") then
                            -- Homme
                            exports['illenium-appearance']:setPedComponents(pPed, {
                                { component_id = 3, drawable = 31, texture = 0 },  -- bras
                                { component_id = 4, drawable = 36, texture = 0 },  -- pantalon
                                { component_id = 6, drawable = 27, texture = 0 },  -- chaussures
                                { component_id = 8, drawable = 59, texture = 0 },  -- t-shirt
                                { component_id = 11, drawable = 89, texture = 1 }, -- torso/jacket
                            })
                            exports['illenium-appearance']:setPedProps(pPed, {
                                { prop_id = 0, drawable = 5, texture = 0 }, -- casque
                            })
                        else
                            -- Femme
                            exports['illenium-appearance']:setPedComponents(pPed, {
                                { component_id = 3, drawable = 68, texture = 0 }, -- bras
                                { component_id = 4, drawable = 30, texture = 2 }, -- pantalon
                                { component_id = 6, drawable = 26, texture = 0 }, -- chaussures
                                { component_id = 8, drawable = 36, texture = 0 }, -- t-shirt
                                { component_id = 11, drawable = 0, texture = 11 }, -- torso/jacket
                                { component_id = 2, drawable = 5, texture = 0 },  -- cheveux
                            })
                            exports['illenium-appearance']:setPedProps(pPed, {
                                { prop_id = 0, drawable = 19, texture = 0 }, -- casque
                            })
                        end
        ...
        elseif InDutty or (IsDutty and GetSelectedPedWeapon(pPed) ~= HashBag) or Route then
            IsInAction = false
            TriggerEvent('illenium-appearance:client:reloadSkin') -- 🔹 Reset skin Illenium
            ClothesChanged = false
