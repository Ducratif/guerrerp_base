...
function MiniGame(options)
    ...
    local requiredKey = math.random(1, 2) == 1 and 38 or 73 -- E ou X
    local label = requiredKey == 38 and "~g~[E]~s~ pour attraper" or "~r~[X]~s~ pour éviter la casse"
    
    ESX.ShowHelpNotification(label)

    while GetGameTimer() - startTime < timeout do
        if IsControlJustPressed(0, requiredKey) then
            success = true
            break
        end
        Wait(0)
