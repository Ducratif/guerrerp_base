...
AddEventHandler('chatMessage', function(author, ctype, text)
    ...
    -- Forcer le type de `ctype` à une string s’il ne l’est pas
    local ctypeStr = type(ctype) == "string" and ctype or "normal"

    SendNUIMessage({
        type = 'ON_MESSAGE',
        message = {
            template = '<div class="chat-message ' .. ctypeStr ..
...
RegisterNUICallback('chatResult', function(data, cb)
    ...
    if not data.canceled then
        ...
        TriggerServerEvent('_chat:messageEntered', playerName, {255, 255, 255}, data.message)
