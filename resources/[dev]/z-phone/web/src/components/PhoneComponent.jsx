...
const PhoneComponent = ({ isShow }) => {
  ...
  try {
    ...
    const let result = response.data;
    if (!result) {
      console.warn("Résultat vide de start-call");
      return;
    }

    if (!result.is_valid) {
      console.warn("Appel invalide :", result.message);
      return;
    }

    console.log("Appel lancé avec succès :", result);
    // TODO: lancer animation ou affichage d'appel ici

  } catch (error) {
    console.error("Erreur JS /start-call :", error);
  }
}}
              ...
              //    const let result = response.data;
              //    if (!result) {
              //      console.warn("Erreur: Résultat vide du /start-call");
              //    } else {
              //      console.log("Appel lancé avec succès :", result);
              //    }
              //  
              //  } catch (error) {
              //    console.error("Erreur JS NUI /start-call:", error);
              //  }
              //}}
