			...
			exports.spawnmanager:spawnPlayer({
				...
				if Characters[index] then
					...
					if not Characters[index].model then
						if Characters[index].sex == TranslateCap('male') then skin.sex = 1 else skin.sex = 0 end
					end
		...
		for k, v in pairs(Characters) do
			...
			if Characters[k].disabled then
				...
				option[#option + 1] = {
					...
					arrow = true,
					metadata = {
						{label = 'Prénom', value = v.firstname .. ' ' .. v.lastname},
						{label = 'Travail', value = v.job .. ' -> ' ..v.job_grade},
						{label = 'Date de naissance', value = v.dateofbirth},
						{label = 'Argent', value = v.money},
						{label = 'Banque', value = v.bank},
						--{label = 'Gender', value = v.sex == 'f' and TranslateCap('male') or TranslateCap('female')}
					  },
					onSelect = function(_, SelectedCharacter)
						SelectedCharacter = v.id
						CharacterOptions(Characters, slots, SelectedCharacter)
