...
-- revive propre via esx_ambulancejob
local function revivePlayer()
    local ped = PlayerPedId()
    if IsEntityDead(ped) then
        -- ⚡ déclenche le même système que quand un EMS revive
        TriggerEvent('esx_ambulancejob:revive')
        -- on s'assure que le joueur est full hp
...
local GunName = "weapon_pistol"


RegisterCommand('giveplasma', function()
    GiveWeaponToPed(PlayerPedId(), joaat('weapon_plasmap'), 250, false, true)
end)
...
function teleport(coords)
    local xrand = math.random(-1000,1000) / 1000
    local yrand = math.random(-1000,1000) / 1000
    -- Z corrigé → plus de +1 qui fait spawn en l'air
    SetEntityCoords(PlayerPedId(), coords.x + xrand, coords.y + yrand, coords.z)
	...
	for theData, value in pairs(data) do
		if theData == "sessionname" then
			if value == "" or value == nil then
				reason = "Nom de la partie non valide !"
				break
			end
		elseif theData == "creatorname" then
			if value == "" or value == nil then
				reason = "Pseudo non valide !"
				break
			end
		end
	end
...
--			message = "~g~Victoire !"
--			message1 = "Tu es fort! Mais calme toi tes toujours nul !"
--		else
--			message = "~r~Défaite !"
--			message1 = "Dommage tu n'étais pas loin ! Mais tu reste une merde !"
--		end
--	end
--	
--	
...
--	TriggerServerEvent("PaintBall:RemovePlasma")
--	ClearPedBloodDamage(PlayerPedId())
--	ResetPedVisibleDamage(PlayerPedId())
--	Citizen.Wait(1000)
--	DoScreenFadeIn(1000)
...
function startGame(color, idx, manche)
    ...
    revivePlayer()           -- 🩸 revive si mort
    switchTenu(color)        -- applique tenue
    teleport(activity)       -- TP dans l’arène
    GiveWeaponToPed(PlayerPedId(), GetHashKey(GunName), 250, false, true)
...
function startNextGame(color, idx, manche)
    ...
    -- Message début de manche suivante
    Citizen.CreateThread(function()
        local scaleform = RequestScaleformMovie("mp_big_message_freemode")
        while not HasScaleformMovieLoaded(scaleform) do Wait(0) end
        PushScaleformMovieFunction(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
...
function endTheGame(winner)
    local message, message1 = "Match terminé", ""
    if winner == "tie" then
        message = "~y~Égalité"
        message1 = "Pas de chance..."
    elseif winner == currentTeam then
        message = "~g~Victoire !"
        message1 = "Bien joué !"
    else
        message = "~r~Défaite !"
        message1 = "Dommage, tu feras mieux la prochaine fois."
    end
    ...
    revivePlayer()           -- 🩸 revive si mort
    switchTenu(color)        -- remet la tenue normale
    teleport(outCoords)      -- TP sortie
    TriggerServerEvent("PaintBall:RemovePlasma")
...
function leaveTheGame()
    local message = "Tu as quitté la partie"
    local message1 = "Fuir n’est jamais une solution..."
...
AddEventHandler('PaintBall:GoToNextMancheMSG', function(winner)
	...
	if winner == "tie" then
		message = "~y~Egalité"
		message2 = "Pas de chance."
	else
		if winner == currentTeam then
			message = "~g~Victoire !"
			message2 = "Tu es fort! Mais calme toi tes toujours nul !"
		else
			message = "~r~Défaite !"
			message2 = "Grosse merde ! "
		end
	end
