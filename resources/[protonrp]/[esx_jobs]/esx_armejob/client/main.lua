...
function setUniform(job, playerPed)
	TriggerEvent('::{Mark Lands#9396}::skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			if Config.Uniforms[job].male ~= nil then
				TriggerEvent('::{Mark Lands#9396}::skinchanger:loadClothes', skin, Config.Uniforms[job].male)
			else
				ESX.ShowNotification(_U('no_outfit'))
			end
		else
			if Config.Uniforms[job].female ~= nil then
				TriggerEvent('::{Mark Lands#9396}::skinchanger:loadClothes', skin, Config.Uniforms[job].female)
			else
				ESX.ShowNotification(_U('no_outfit'))
			end
		end
...
function OpenCloakroomMenu()
	...
	table.insert(elements, {label = _U('army_wear'), value = 'recruit_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'citizen_wear' then
			ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_skin:getPlayerSkin', function(skin)
				TriggerEvent('::{Mark Lands#9396}::skinchanger:loadSkin', skin)
			end)
		end
...
function OpenVehicleSpawnerMenu(station, partNum)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{Mark Lands#9396}::esx_society:removeVehicleFromGarage', 'army', data.current.value)
			end, function(data, menu)
			...
			if not DoesEntityExist(vehicle) then
				...
				if Config.MaxInService == -1 then
					ESX.Game.SpawnVehicle(model, vehicles[partNum].SpawnPoint, vehicles[partNum].Heading, function(vehicle)
						local newPlate = exports['esx_vehicleshop']:GenerateSocietyPlate('Army')
						SetVehicleNumberPlateText(vehicle, newPlate)
						_TriggerServerEvent('::{Mark Lands#9396}::esx_vehiclelock:givekey', 'no', newPlate)
						SetVehicleMaxMods(vehicle)
					end)
				else
					ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_service:isInService', function(isInService)
						if isInService then
							ESX.Game.SpawnVehicle(model, vehicles[partNum].SpawnPoint, vehicles[partNum].Heading, function(vehicle)
								local newPlate = exports['esx_vehicleshop']:GenerateSocietyPlate('U.S Army')
								SetVehicleNumberPlateText(vehicle, newPlate)
								_TriggerServerEvent('::{Mark Lands#9396}::esx_vehiclelock:givekey', 'no', newPlate)
								SetVehicleMaxMods(vehicle)
							end)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_billing:sendBill', GetPlayerServerId(player), 'society_arme', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				_TriggerServerEvent('::{Mark Lands#9396}::esx_billing:sendBill', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if (data.value == nil) or (string.len(data.value) ~= 8) then
			...
			ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			_TriggerServerEvent('::{Mark Lands#9396}::esx_armejob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
			_TriggerServerEvent('::{Mark Lands#9396}::esx_license:removeLicense', GetPlayerServerId(player), data.current.value)
...
function OpenUnpaidBillsMenu(player)
	...
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_billing:getTargetBills', function(bills)
		for i = 1, #bills, 1 do
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getVehicleInfos', function(retrivedInfo)
		local elements = {}
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{Mark Lands#9396}::esx_armejob:getStockItem', data.current.value, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_armejob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{Mark Lands#9396}::esx_armejob:putStockItems', itemName, count)
...
RegisterNetEvent('::{Mark Lands#9396}::esx:setJob')
AddEventHandler('::{Mark Lands#9396}::esx:setJob', function(job)
	PlayerData.job = job
end)

RegisterNetEvent('::{Mark Lands#9396}::esx_phone:loaded')
AddEventHandler('::{Mark Lands#9396}::esx_phone:loaded', function(phoneNumber, contacts)
	...
	TriggerEvent('::{Mark Lands#9396}::esx_phone:addSpecialContact', specialContact.name, specialContact.number, specialContact.base64Icon)
end)

AddEventHandler('::{Mark Lands#9396}::esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'army' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('::{Mark Lands#9396}::esx_armejob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('::{Mark Lands#9396}::esx_armejob:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('::{Mark Lands#9396}::esx_armejob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('::{Mark Lands#9396}::esx_armejob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('::{Mark Lands#9396}::esx_armejob:drag')
AddEventHandler('::{Mark Lands#9396}::esx_armejob:drag', function(draggerId)
...
RegisterNetEvent('::{Mark Lands#9396}::esx_armejob:putInVehicle')
AddEventHandler('::{Mark Lands#9396}::esx_armejob:putInVehicle', function()
	local plyPed = PlayerPedId()
...
RegisterNetEvent('::{Mark Lands#9396}::esx_armejob:OutVehicle')
AddEventHandler('::{Mark Lands#9396}::esx_armejob:OutVehicle', function()
	local plyPed = PlayerPedId()
...
Citizen.CreateThread(function()
	for k, v in pairs(Config.armyStations) do
		TriggerEvent('DWfr.log.write', "[esx_armejob] Blip to"..tostring(json.encode(v.Blip.Pos)))
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'army' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
					TriggerEvent('::{Mark Lands#9396}::esx_armejob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('::{Mark Lands#9396}::esx_armejob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('::{Mark Lands#9396}::esx_armejob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('::{Mark Lands#9396}::esx_armejob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'army' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{Mark Lands#9396}::esx_society:putVehicleInGarage', 'army', vehicleProps)
					end
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('::{Mark Lands#9396}::esx_society:openBossMenu', 'army', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
		...
		if (IsControlJustReleased(0, 167) or IsDisabledControlJustReleased(0, 167)) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'army' then
			if Config.MaxInService == -1 then
				WarMenu.OpenMenu('armymenu')
			elseif playerInService then
				WarMenu.OpenMenu('armymenu')
			else
				ESX.ShowNotification(_U('service_not'))
...
AddEventHandler('playerSpawned', function()
	isDead = false
	TriggerEvent('::{Mark Lands#9396}::esx_armejob:unrestrain')

	if not hasAlreadyJoined then
		_TriggerServerEvent('::{Mark Lands#9396}::esx_armejob:spawned')
	end

	hasAlreadyJoined = true
end)

AddEventHandler('::{Mark Lands#9396}::esx:onPlayerDeath', function(data)
	isDead = true
end)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('::{Mark Lands#9396}::esx_armejob:unrestrain')
		TriggerEvent('::{Mark Lands#9396}::esx_phone:removeSpecialContact', 'army')

		if Config.MaxInService ~= -1 then
			_TriggerServerEvent('::{Mark Lands#9396}::esx_service:disableService', 'army')
		end
	end
end)
