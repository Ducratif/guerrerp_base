	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('::{Mark Lands#9396}::esx_society:removeVehicleFromGarage', 'bahamas', vehicleProps)
			end, function(data, menu)
	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('craftmenu') then
			if WarMenu.Button(_U('jagerbomb')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'jagerbomb')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('golem')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'golem')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('whiskycoca')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'whiskycoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkaenergy')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'vodkaenergy')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkafruit')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'vodkafruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumfruit')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'rhumfruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('teqpaf')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'teqpaf')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumcoca')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'rhumcoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mojito')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'mojito')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mixapero')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'mixapero')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('metreshooter')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'metreshooter')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('jagercerbere')) then
				_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:craftingCoktails', 'jagercerbere')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
		...
		if player ~= -1 and distance <= 3.0 then
			...
			if amount == nil then
				...
				_TriggerServerEvent('::{Mark Lands#9396}::esx_billing:sendBill', GetPlayerServerId(player), 'society_bahamas', _U('billing'), amount)
			end
		else
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:getStockItem', itemName, count)
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:putStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenGetFridgeStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:getFridgeStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:getFridgeStockItem', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:putFridgeStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:getVaultWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_get_weapon', {
			...
			ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:removeVaultWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_put_weapon', {
		...
		ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_bahamasjob:addVaultWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value)
	end, function(data, menu)
	end)
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bahamas_shop', {
		...
		_TriggerServerEvent('::{Mark Lands#9396}::esx_bahamasjob:buyItem', data.current.value, data.current.price, data.current.label)
	end, function(data, menu)
	end)
end
...
AddEventHandler('::{Mark Lands#9396}::esx_bahamasjob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{Mark Lands#9396}::esx_bahamasjob:hasExitedMarker', function(zone)
		CurrentAction = nil
		ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{Mark Lands#9396}::esx_bahamasjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{Mark Lands#9396}::esx_bahamasjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{Mark Lands#9396}::esx_society:putVehicleInGarage', 'bahamas', vehicleProps)
					else
						if
							GetEntityModel(vehicle) == `rentalbus`
						then
							_TriggerServerEvent('::{Mark Lands#9396}::esx_service:disableService', 'bahamas')
						end
					end
				...
				if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
					...
					TriggerEvent('::{Mark Lands#9396}::esx_society:openBossMenu', 'bahamas', function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, options)
...
AddEventHandler('::{Mark Lands#9396}::esx_bahamasjob:teleportMarkers', function(position)
	...
	while true do
		...
		if IsJobTrue() then
			...
			if IsControlJustReleased(0, 38) and isInPublicMarker then
				TriggerEvent('::{Mark Lands#9396}::esx_bahamasjob:teleportMarkers', position)
			end
