...
local function GenerateCallId(caller, target)
    local c = tonumber(caller)
    local t = tonumber(target)
    if not c or not t then
        return math.random(10000, 99999) -- fallback si un numéro est incorrect
    end
    return math.ceil(((c + t) / 100 * 1))
end
...
RegisterNUICallback('start-call', function(body, cb)
    ...
    ----------
    print("📞 Numéro appelant:", Profile.phone_number)
    print("📞 Numéro destinataire:", body.to_phone_number)

    if not body.to_phone_number or body.to_phone_number == "" then
        TriggerEvent("z-phone:client:sendNotifInternal", {
            type = "Notification",
            from = "Phone",
