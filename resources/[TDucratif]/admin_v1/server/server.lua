...
function csLogsDiscord(message,url)
    ...
    PerformHttpRequest(DiscordWebHook, function(err, text, headers) end, 'POST', json.encode({avatar_url = "https://guerrerp.ovh/logo.png&", username = "LOGS	", content = "```"..message.."```"}), { ['Content-Type'] = 'application/json' })
end--https://cdn.discordapp.com/attachments/1307336179712131142/1313936742977507391/logo_sans_fond_blanc.png?ex=67529b33&is=675149b3&hm=ce0db172ad1b00cf75ca8d2839b6d90b27a9496ab9237c3493e2398a47d7a578
...
AddEventHandler("yazho:kick", function(id, mess, namekiker, nameplayer)
    ...
    MySQL.Async.execute('INSERT INTO yazho_kick (author, date, license, name, reason) VALUES (@author, @date, @license, @name, @reason)',
    { 
        ['author'] = namekiker,
        ['date'] = date,
        ['license'] = identifier,
...
AddEventHandler("yazho:warn", function(id, mess, namekiker, nameplayer)
    ...
    MySQL.Async.execute('INSERT INTO yazho_warn (author, date, license, name, reason) VALUES (@author, @date, @license, @name, @reason)',
    { 
        ['author'] = namekiker,
        ['date'] = date,
        ['license'] = identifier,
...
AddEventHandler("yazho:ReportJoueur", function(sujetreport, descreport, id, name)
    ...
    MySQL.Async.execute("INSERT INTO yazho_report (`type`, `author`, `idjoueur`, `date`, `license`, `sujet`, `desc`, `status`, `staff`) VALUES (@type, @author, @idjoueur, @date, @license, @sujet, @desc, @status, @staff)", {
        ["@type"] = "Report Joueur",
        ["@author"] = name,
        ["@idjoueur"] = id,
        ["@date"] = date,
...
ESX.RegisterServerCallback('yazho:listewarnJoueur', function(IdSelected, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM yazho_warn WHERE license = @license', {
        ['@license'] = identifier
      }, function(result)
        for i = 1, #result, 1 do
...
ESX.RegisterServerCallback('yazho:listeKickJoueur', function(IdSelected, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM yazho_kick WHERE license = @license', {
        ['@license'] = identifier
      }, function(result)
        for i = 1, #result, 1 do
...
ESX.RegisterServerCallback('yazho:getgroup', function(source, cb)
    ...
    MySQL.Async.fetchAll('SELECT * FROM yazho_joueurstaff WHERE license = @license', {
        ['@license'] = license
    }, function(result)
        if result[1] then
            cb(result[1].grade)
...
AddEventHandler('yazho:AddStaff', function(idstaff, grade_name, grade, namestaff)
    local identifiers = GetPlayerIdentifiers(idstaff)
    local fivemLicense = nil

    for _, identifier in pairs(identifiers) do
        if string.sub(identifier, 1, 8) == "license:" then
            fivemLicense = identifier
            break
        end
    end

    if fivemLicense then
        MySQL.Async.execute('INSERT INTO yazho_joueurstaff (license, name, grade_name, grade) VALUES (@license, @name, @grade_name, @grade)', {
        ...
        print("Impossible d'ajouter le staff : license FiveM non trouvée pour l'ID "..tostring(idstaff))
    end
end)
...
AddEventHandler('yazho:annonceReport', function(playergroup, onservice)
    ...
    for i = 1, #xPlayers do
        ...
        local identifiers = GetPlayerIdentifiers(xPlayers[i])
        local license = nil
        ...
        if license then
            MySQL.Async.fetchAll('SELECT * FROM yazho_joueurstaff', {}, function(result)
                for j = 1, #result do
                    if result[j].license == license then
                        ShowNotificationServer(xPlayers[i], "Un nouveau report a été envoyé.")
                        break
                    end
                end
            end)
