...
local function openShop(mechanicId, shopIndex)
  ...
  for index, v in pairs(shopItems) do
    table.insert(options, {
      title = v.label,
      icon = "box",
      --description = ("Acheter pour %d %s"):format(v.price, Config.Currency),
      description = ("Acheter pour %d $"):format(v.price, Config.Currency),
    ...
    for shopIndex, shop in ipairs(mechanicConfig.shops) do
      ...
      if shop.usePed then
        ...
        local zoneData = {
          ...
          debug = Config.Debug,
          onEnter = function()
            Framework.Client.ShowTextUI("Appui sur [E] pour " .. shop.name)
          end,
          onExit = function()
    ...
    for stashIndex, stash in ipairs(mechanicConfig.stashes) do
      ...
      if stash.usePed then
        ...
        local zoneData = {
          ...
          debug = Config.Debug,
          onEnter = function()
            Framework.Client.ShowTextUI("Appui sur [E] pour " .. stash.name)
          end,
          onExit = function()
            Framework.Client.HideTextUI()
          end,
