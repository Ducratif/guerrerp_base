...
CreateThread(function()
    if (GetCurrentResourceName() ~= 'LeDjo_Garage') then
        return
    end
    CrearBlips()
end)
    ...
    for k, v in pairs(Garage.Garages) do
        if not v.impound then
            if Garage.Target then
                    ...
                    options = {
                        {
                            ...
                            onSelect = function()
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    TriggerEvent('LeDjo_Garage:Notification',
                                        locale('SalirVehiculo'))
                                else
                                    TriggerEvent('LeDjo_Garage:garage',
                ...
                local options = {
                    {
                        name = 'LeDjo_Garage:TargetGuardar',
                        ...
                        onSelect = function()
                            ...
                            if closet then
                               local plate = string.gsub(GetVehicleNumberPlateText(closet), "^%s*(.-)%s*$", "%1")
                                ...
                                TriggerServerEvent('LeDjo_Garage:GuardarVehiculo', plate, json.encode(vehicleProps), k,VehToNet(closet))
                            else
                                TriggerEvent('LeDjo_Garage:Notification', locale('mascerca'))
                            end
                        end
                    },
                }
                local optionNames = { 'LeDjo_Garage:TargetGuardar' }
                                        ...
                                        if ESX.PlayerData.job.name == v.job then
                                            if SyGargeTypeCar(Sy.GetClase()) == v.type then
                                                if Sy.GetProps() == nil then
                                                    return TriggerEvent('LeDjo_Garage:Notification',
                                                        locale('EnUnVeh'))
                                                end
                                                if Garage.CarKeys then
                    ...
                    options = {
                        {
                            ...
                            label = k,
                            onSelect = function()
                                TriggerEvent('LeDjo_Garage:garageImpound',
...
AddEventHandler('LeDjo_Garage:garage', function(data)
    local garagemenu = {}
    local vehicles = lib.callback.await('LeDjo_Garage:getOwnerVehicles')
    ...
    for i = 1, #vehicles do
        ...
        if Garage.SharedGarage then
            ...
            table.insert(garagemenu, {
                ...
                event = 'LeDjo_Garage:VehiculoSeleccionado',
            })
        else
    ...
    lib.showContext('LeDjo_Garage:MenuCarList')
end)


AddEventHandler('LeDjo_Garage:VehiculoSeleccionado', function(data)
    local select = {}
    if data.stored == 1 then
        table.insert(select, {
                ...
                for j = 1, #data.spawn do
                    ...
                    if ESX.Game.IsSpawnPointClear(pos, 2.0) then
                        if Garage.CarKeys then
                            ...
                            print('Véhicule sorti + clé donner')
                        end
                        TriggerServerEvent('LeDjo_Garage:RetirarVehiculo', data.plate, data.garage, pos, hea, props, model)
                        SpawPos = true
                        break
                    end
                end
                if not SpawPos then
                    TriggerEvent('LeDjo_Garage:Notification', locale('NoSpawn'))
                end
            end
        })
            ...
            if data.id == data.duen then
                table.insert(select, {
                    ...
                    onSelect = function()
                        ...
                        TriggerServerEvent('LeDjo_Garage:CompartirAmigo', input[1], input[2], data.plate)
                    end
                })
                ...
                if data.amigos ~= nil then
                    table.insert(select, {
                        ...
                        onSelect = function()
                                    ...
                                    onSelect = function()
                                        local alert = lib.alertDialog({
                                            ...
                                            buttons = {
                                                {
                                                    text = locale('elimi'),
                                                    event = 'LeDjo_Garage:EliminarAmigo',
                                        ...
                                        if alert == "cancel" then
                                            ...
                                            TriggerServerEvent('LeDjo_Garage:EliminarAmigo', ami.name, data.plate)
                                        end
                                    end
                                })
                            end
                            lib.registerContext({
                                id = 'menu_amigos',
                                menu = 'LeDjo_Garage:VehiculoSeleccionado',
                                title = locale('ListaAmigos'),
                                options = amigos
                            })
        ...
        if data.id == data.duen then
            ...
            table.insert(select, {
                ...
                onSelect = function()
                    TriggerServerEvent('LeDjo_Garage:MandarVehiculoImpound', data.plate, data.impo)
                end
            })
            if Garage.ShareCarFriend then
                if data.amigos ~= nil then
    ...
    lib.registerContext({
        id = 'LeDjo_Garage:VehiculoSeleccionado',
        menu = 'LeDjo_Garage:MenuCarList',
        title = data.marca .. ' - ' .. data.name,
        options = select
    })
    lib.showContext('LeDjo_Garage:VehiculoSeleccionado')
end)


AddEventHandler('LeDjo_Garage:garageImpound', function(nata)
    local ImpoundMenu = {}
    local vehicles = lib.callback.await('LeDjo_Garage:getOwnerVehicles')
    local bank = lib.callback.await('LeDjo_Garage:getBankMoney')
        ...
        if data.pound == '1' and garageName == data.parking then
            ...
            table.insert(ImpoundMenu, {
                ...
                onSelect = function()
                    ...
                    if not input[1] then
                        return TriggerEvent('LeDjo_Garage:Notification', locale('metododepagos'))
                    end
                        ...
                        if bank.money >= price then
                            for j = 1, #spawn do
                                ...
                                if ESX.Game.IsSpawnPointClear(vector3(v.x, v.y, v.z), 3.0) then
                                    TriggerServerEvent('LeDjo_Garage:RetirarVehiculoImpound', plate, input[1], price,
                                        data, pos, hea, model)
                                    SpawPos = true
                                    if Garage.CarKeys then
    ...
    lib.showContext('LeDjo_Garage:ImpoundMenu')
end)
        ...
        while true do
            ...
            if IsPedInAnyVehicle(PlayerPedId(), false) and not inVeh then
                ...
                if driver then
                    ...
                    local vehicles = lib.callback.await('LeDjo_Garage:owner_vehicles')
                    if vehicles then
                        for i = 1, #vehicles do
                            local data = vehicles[i]
                            if plate == data.plate then
                                ...
                                TriggerServerEvent('LeDjo_Garage:AgregarKilometros', plate, data.mileage)
                                inVeh = false
                            end
                        end 
                    end
