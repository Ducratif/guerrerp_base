  ...
  window.addEventListener("message", function (event) {
    if (event.data.type == "menu") {
      ...
      document.querySelector(".name .desc").innerHTML =
        "<i class='fa-light fa-id-card'></i>Information: <span>" +
      ...
      document.querySelector(".cash .desc").innerHTML =
        "<i class='fa-light fa-money-bills-simple'></i>Liquide: <span>" +
        formatter.format(event.data.money) +
        "</span>";
      document.querySelector(".bank .desc").innerHTML =
        "<i class='fa-light fa-building-columns'></i>Banque: <span>" +
...
function joinDiscord() {
  window.invokeNative("openUrl", "https://discord.gg/kpD8pQBBWm");
}
