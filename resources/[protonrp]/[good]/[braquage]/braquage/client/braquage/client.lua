...
local menuDescByHouseState = {
    [true] = {title = "Cambrioler la maison", desc = "Appuyez pour commencer le cambriolage (Attention au hendek de GuerreRP)", style = {RightLabel = "→→"}},
    [false] = { title = "Repassez plus tard", desc = "Cette maison a été récemment vandalisée par un timoté, repassez plus tard..", style = {} },
}
...
Citizen.CreateThread(function()
    TriggerServerEvent("ducratif_braquage:getHousesStates")

    while not housesDataSync do Citizen.Wait(100) end

    RMenu.Add("ducratif_braquage", 'robb', RageUI.CreateMenu("Cambriole","Cambriolage de maisons"))
    RMenu:Get('ducratif_braquage', 'robb'):SetRectangleBanner(0, 0, 0, 240); --Menu noir
    initMenuLoop()
            ...
            for index,house in pairs(robberiesConfiguration.houses) do
                ...
                if thisHouseDistance <= 15 then
                    ...
                    if thisHouseDistance <= 1 then
                        closeToHouse = index
                        if not RageUI.Visible(RMenu:Get("ducratif_braquage",'robb')) then
                            RageUI.Text({message = "Appuyez sur ~o~[E]~s~ pour vérifier la serrurre",time_display = 1})
                        end
                    ...
                    elseif thisHouseDistance > 1 and RageUI.Visible(RMenu:Get("ducratif_braquage",'robb')) then
                        RageUI.Visible(RMenu:Get("ducratif_braquage",'robb'), false)
                    end
                else
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            TriggerServerEvent("ducratif_braquage:reward", finalReward)
                            TriggerServerEvent("ducratif_braquage:money", finalReward)
                            RemoveBlip(resellerBlip)
                            Citizen.Wait(500)
                            isRobbing = false
                            currentRobbingData = {houseIndex = nil, step = 0, data = {}}
                            robbItems = {}
                            --ESX.ShowAdvancedNotification("Bravo","~r~Cambriolage","Félicitations tu as reçus ~r~"..finalReward.."$ ~s~ d'argent sale grace aux objects que tu as volé! Je te recontacterai (Jamais j'ai piscine)","CHAR_DEVIN",9)
                            exports.ox_lib:notify({
                                title = 'Cambriolage',
                                description = ("Félicitations tu as reçus "..finalReward.."$ d'argent sale grace aux objects que tu as volé! Je te recontacterai (Jamais j'ai piscine)"),
                                type = 'success',
...
function terminate()
    ...
    if #robbItems ~= 0 then
       -- ESX.ShowAdvancedNotification("Bravo","~r~Cambriolage","Bravo pour ton cambriolage, tu as ~b~"..#robbItems.." objects ~s~sur toi. Rends toi devant le reseller pour les revendres!","CHAR_DEVIN",9)
        exports.ox_lib:notify({
            title = 'Cambriolage',
            description = ("Bravo pour ton cambriolage, tu as "..#robbItems.." objects sur toi. Rends toi devant le reseller pour les revendres!"),
            type = 'success',
        ...
        exports.ox_lib:notify({
            title = 'LA CONTE TA RACE',
            description = ("T'es sérieux toi ? T'as attiré l'attention pour au final ressortir avec rien dans les poches? Jvais te goumer"),
            type = 'error',
    ...
    Citizen.SetTimeout((1000*60)*robberiesConfiguration.houses[currentRobbingData.houseIndex].timeToGoToReseller, function()
        if not itemsSold then
            ...
            exports.ox_lib:notify({
                title = 'Vas te faire',
                description = ("T'es vraiment une merde... Tu fais déplacer un reseller pour au final pas te pointer? J'en ai trop vu, à la prochaine!"),
                type = 'error',
...
function initMenuLoop()
    Citizen.CreateThread(function()
        while true do
            RageUI.IsVisible(RMenu:Get("ducratif_braquage",'robb'),true,true,true,function()
                RageUI.ButtonWithStyle(menuDescByHouseState[housesData[closeToHouse].state].title,menuDescByHouseState[housesData[closeToHouse].state].desc, menuDescByHouseState[housesData[closeToHouse].state].style, housesData[closeToHouse].state, function(Hovered,Active,Selected)
                    if Selected then
                        ...
                        if not thisHouseAlreadyRobbed then
                            ...
                            TriggerServerEvent("ducratif_braquage:houseRobbed", closeToHouse)
                            isRobbing = true
        ...
        while isRobbing and stopreason == nil do
            if not copsCalleds then
                if currentRobbingData.data.copsCalledAfter <= 0 then
                    ...
                    exports.ox_lib:notify({
                        title = 'Central de police 911 Tracker',
                        description = ("Un témoin oculaire vous a aperçu et a contacté les forces de l'ordre, la police a donc été prévenue et connait votre localisation. EN GROS COURS MICHEL !"),
                        type = 'error',
                        
                    })
                    TriggerServerEvent("ducratif_braquage:callThePolice", currentRobbingData.houseIndex)
                else
...
function alterMenuVisibility_()
    RageUI.Visible(RMenu:Get("ducratif_braquage",'robb'), not RageUI.Visible(RMenu:Get("ducratif_braquage",'robb')))
end
...
AddEventHandler("ducratif_braquage:getHousesStates", function(array)
    housesData = array
    housesDataSync = true
end)

RegisterNetEvent("ducratif_braquage:initializePoliceBlip")
AddEventHandler("ducratif_braquage:initializePoliceBlip", function(houseIndex,duration)
    ...
    exports.ox_lib:notify({
        title = 'Central de police',
        description = ("Un civil a appellé la police à cause d'un possible cambriolage."),
        type = 'error',
