...
-- Ajout d'un stash personnel
RegisterServerEvent('ox_inventory:AddPersonalStash')
AddEventHandler('ox_inventory:AddPersonalStash', function(name, label, maxWeight, maxValue)
    -- Vérifie si les arguments sont valides
    if not name or not label or not maxWeight or not maxValue then
        print('Erreur: Paramètres manquants pour l\'ajout du stash.')
        return
    end

    -- Création du stash (ici tu peux personnaliser selon tes besoins)
    local stashData = {
        name = name,
        label = label,
        maxWeight = maxWeight,
        maxValue = maxValue,
        items = {}, -- Un tableau vide au début pour les objets du stash
    }

    -- Logique pour stocker ce stash dans la base de données ou en mémoire
    -- Exemple : Ajout dans un tableau global (à remplacer par ta base de données si nécessaire)
    local personalStashes = {}  -- A remplacer par ton système de stockage de stashes (ex: base de données)
    personalStashes[name] = stashData

    -- Retourne un message de succès
    TriggerClientEvent('ox_inventory:stashAdded', source, stashData)

    -- Optionnel : Affichage d'un message dans la console du serveur
    print('Nouveau stash ajouté : ' .. name)
end)

-- Export pour permettre l'appel à partir d'autres ressources
exports('AddPersonalStash', function(name, label, maxWeight, maxValue)
    -- Même logique que pour l'événement
    if not name or not label or not maxWeight or not maxValue then
        print('Erreur: Paramètres manquants pour l\'ajout du stash.')
        return
    end
    ...
    -- Stocke ou enregistre ce stash comme nécessaire
    -- Exemple avec un tableau local (remplacer par un système de stockage persistant)
    local personalStashes = {}
    personalStashes[name] = stashData

    return stashData  -- Retourne le stash créé
end)
...
-- Ajout d'un stash lié à un métier
RegisterServerEvent('ox_inventory:AddJobStash')
AddEventHandler('ox_inventory:AddJobStash', function(name, label, maxWeight, maxValue, isShared, job)
    -- Vérifie si les arguments sont valides
    if not name or not label or not maxWeight or not maxValue or not job then
        print('Erreur: Paramètres manquants pour l\'ajout du stash lié au métier.')
        return
    end
    ...
    local stashData = {
        ...
        maxValue = maxValue,
        isShared = isShared,
        job = job,   -- Le job auquel le stash est lié
        items = {},  -- Un tableau vide au début pour les objets du stash
    }

    -- Logique pour stocker ce stash dans la base de données ou en mémoire
    -- Exemple : Ajout dans un tableau global (à remplacer par ta base de données si nécessaire)
    local jobStashes = {}  -- A remplacer par ton système de stockage de stashes (ex: base de données)
    jobStashes[job] = jobStashes[job] or {}
    table.insert(jobStashes[job], stashData)

    -- Retourne un message de succès
    TriggerClientEvent('ox_inventory:jobStashAdded', source, stashData)

    -- Optionnel : Affichage d'un message dans la console du serveur
    print('Nouveau stash ajouté pour le job ' .. job .. ': ' .. name)
end)

-- Export pour permettre l'appel à partir d'autres ressources
exports('AddJobStash', function(name, label, maxWeight, maxValue, isShared, job)
    ...
    local stashData = {
        ...
        maxWeight = maxWeight,
        maxValue = maxValue,
        isShared = isShared,  -- Indique si le stash est partagé
        job = job,   -- Le job auquel le stash est lié
        items = {},  -- Un tableau vide au début pour les objets du stash
    }
