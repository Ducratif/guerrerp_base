...
CreateThread(function()
    local barber = GetHashKey("s_f_m_fembarber")
    if not HasModelLoaded(barber) then
        RequestModel(barber)
        while not HasModelLoaded(barber) do
            Wait(10)
        end
    end
end)
...
function createBarber()
    Ped = CreatePed(1, barber, 141.48, -1705.59, 29.29 - 0.95, 0.0, true, true)
    SetEntityHeading(Ped, 123.37)
    SetEntityInvincible(Ped, true)
    SetBlockingOfNonTemporaryEvents(Ped, true)
    TaskPedSlideToCoord(Ped, 137.15, -1710.50, 29.3, 205.75, 1.0)
    Citizen.Wait(10000)
