...
Config.WaterMarkInformations = { -- informations displayed in the upper right watermark
    logo = "https://i.ibb.co/7JPMTJvw/mhudlogo.png",
    servername = "GuerreRP",
    discordlink = "discord.gg/kpD8pQBBWm"
}


Config.CountryCode = "fr-FR" -- this is used to format the date you can check country codes here -- https://www.w3schools.com/jsref/jsref_tolocalestring_number.asp
...
Config.MoneyHud = {
    ...
    vip_money = {
        ...
        getMoney = function(this)
            if this.show then
                WaitCore() -- Attend que le core soit chargé
                WaitPlayer() -- Attend que le joueur soit chargé

                -- Charger l'argent VIP au début
                local vipMoney = TriggerCallback("mHud:getPlayerVipMoney")
                LoadPlayerMoney("vip_money", vipMoney)
                print("[CLIENT]Local VipMoney")

                -- Event pour actualiser en temps réel l'argent VIP
                RegisterNetEvent("mHud:VipMoneyAmountChanged")
                AddEventHandler("mHud:VipMoneyAmountChanged", function(amount)
                    print("[CLIENT] LoadPlayerMoney")
                    LoadPlayerMoney("vip_money", amount)
                end)
            end
        end
...
Config.Gift = {
    enable = true,
    text = "<p>Jouer</p> <span>{0} MN</span> <p>et gagner</p> ",
    time = 120, -- x play time to give gift
    rewards = {
        {
            type = "item",
            name = "water",
