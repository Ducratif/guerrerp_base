	...
	if not alreadyIn then
		if #nbPlayer == nil then
			...
			if #nbPlayer < tonumber(currentPaintBallSession[idx].nbpersequip) then
				...
				TriggerClientEvent("PaintBall:Notif",player,"Vous avez rejoint l’équipe rouge","info")
			else
...
AddEventHandler('PaintBall:StartTheGame', function(idx)
    ...
    -- Donne l'arme Plasma au créateur
...
function sendToPlayerTheStart(idx)
	...
	--end
	for k,v in pairs(currentPaintBallSession[idx].EquipA) do
	    TriggerClientEvent("PaintBall:GoForStartTheGame", v, "blue", idx, currentPaintBallSession[idx].manche)
	    SetPlayerRoutingBucket(v, tonumber(idx))
	    exports.ox_inventory:AddItem(source, 'weapon_pistol ', 1)
	end

	for k,v in pairs(currentPaintBallSession[idx].EquipB) do
	    TriggerClientEvent("PaintBall:GoForStartTheGame", v, "red", idx, currentPaintBallSession[idx].manche)
	    SetPlayerRoutingBucket(v, tonumber(idx))
	    exports.ox_inventory:AddItem(source, 'weapon_pistol ', 1)
	end
...
RegisterNetEvent("PaintBall:RemovePlasma", function()
    local src = source
    exports.ox_inventory:RemoveItem(src, "weapon_pistol ", 1)
end)
