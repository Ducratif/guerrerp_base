...
function NPCImpoundGarage(impound)
    ...
    if vehicle then
        if Garage.NpcImpound.NPCAnim then
            ...
            TriggerServerEvent('LeDjo_Garage:MandarVehiculoImpound', plate, impound)
            TriggerEvent('LeDjo_Garage:Notification', locale('impound1', plate))
        else
            Wait(5000)
            DeleteEntity(vehicle)
            TriggerServerEvent('LeDjo_Garage:MandarVehiculoImpound', plate, impound)
        ...
        TriggerEvent('LeDjo_Garage:Notification', locale('no_veh_nearby'))
    end
end
...
RegisterCommand(Garage.NpcImpound.Command, function()
        ...
        if ESX.PlayerData.job.name == Garage.NpcImpound.jobs[i] then
            ...
            if Sy.CarCloset({ dist = 7, pedcar = true }) then
                ...
                if lib.progressBar({
                    ...
                    TriggerEvent('LeDjo_Garage:Notification', locale('cancelado'))
                end
            else
                TriggerEvent('LeDjo_Garage:Notification', locale('no_veh_nearby'))
            end
    ...
    if not jobAllowed then
        TriggerEvent('LeDjo_Garage:Notification', locale('impound3'))
    end
end)

AddEventHandler('LeDjo_Garage:NPCImpound', function()
    if Sy.CarCloset({ dist = 7, pedcar = true }) then
        local input
        local imp = {}
        for k, v in pairs(Garage.Garages) do
