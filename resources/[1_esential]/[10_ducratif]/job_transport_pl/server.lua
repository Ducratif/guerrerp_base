...
AddEventHandler("job_transport_pl:giveMoney", function(amount, xp)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then
        print(("[job_transport_pl] Player %s not found"):format(source))
        return
    end
    ...
    if amount > 0 then
        xPlayer.addMoney(amount) -- ou addAccountMoney('cash', amount) si tu préfères
        TriggerClientEvent('esx:showNotification', source, ("Vous avez reçu ~g~%s€ ~s~en liquide."):format(amount))
    end

    -- XP (sanitisation + clamp)
    local giveXp = math.max(0, math.floor(tonumber(xp) or 0))
    if giveXp > 0 then
        ExecuteCommand(('esxp_give %s %d'):format(source, giveXp))
    end
end)
