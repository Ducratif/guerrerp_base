...
function deleteRank(jobName, rankId) {
	deleteDialog("Supprimer le grade", `Êtes vous sur de supprimer ce grade ?`, function () {
...
function fillJobRanks(jobName, ranks) {
	...
	var mainRowContent = $(`
		<th scope="col">Grade Name (dans la base)</th>
		<th scope="col">Hauteur du grade (0 = bas)</th>
		<th scope="col">Nom du grade</th>
		<th scope="col">Salaire</th>
	`)

	if (!hasAnyRank) {
		mainRowContent = $(`<th scope="col">Aucun grade pour ce job pour le moment</th>`)
	}
	...
	ranks.forEach(rank => {
		...
		jobRank.click(function () {
			inputDialog("Modification du grade", [
				{ id: "rankName", label: "Grade Name (dans la base):", default: rank.name, type: "text", isId: true },
				{ id: "rankGrade", label: "Hauteur du grade (0 = bas):", default: rank.grade, type: "number" },
				{ id: "rankLabel", label: "Nom du grade:", default: rank.label, type: "text" },
				{ id: "rankSalary", label: "Salaire du grade:", default: rank.salary, type: "number" },
				{
					id: "deleteRank", label: "Supprimer le grade", type: "button", buttonType: "btn-danger",
...
function createRank(jobName) {
	inputDialog("Crée un nouveau grade", [
		{ id: "rankName", label: "ID Du grade:", type: "text", isId: true },
		{ id: "rankGrade", label: "Hauteur du grade (0 = bas):", type: "number" },
		{ id: "rankLabel", label: "Nom du grade:", type: "text" },
		{ id: "rankSalary", label: "Salaire du grade:", type: "number" },
	], function (data) {
...
$("#create-job-btn").click(function () {
	inputDialog("Crée un nouveau job", [
		{ id: "jobLabel", label: "Nom du job:" },
		{ id: "jobName", label: "Nom du job (id):", isId: true },
	], function (data) {
...
function deleteJob(jobName) {
	deleteDialog("Supprimer le job", `Êtes vous sur de supprimer le job ${jobName}?`, function () {
...
function fillJobs(jobs) {
	...
	for (const [_, job] of Object.entries(jobs)) {
		...
		dropdownEditJob.click(function () {
			...
			inputDialog("Modifier le job", [
				{ id: "jobName", label: "Nom du job (id):", default: jobName, type: "text", isId: true },
				{ id: "jobLabel", label: "Nom du job:", default: jobLabel, type: "text" },
				{ id: "enableBilling", label: "Activer la facturation", default: enableBilling, type: "switch" },
				{ id: "canRob", label: "Peut-on voler/rechercher des joueurs ?", default: canRob, type: "switch" },
				{ id: "canHandcuff", label: "Peut-on menotter les joueurs ?", default: canHandcuff, type: "switch" },
				{ id: "whitelisted", label: "Sur liste blanche", default: whitelisted, type: "switch" },
				{ id: "canLockpickCars", label: "Peut-on crocheter les voitures ?", default: canLockpickCars, type: "switch" },
				{ id: "canWashVehicles", label: "Peut-on laver les véhicules ?", default: canWashVehicles, type: "switch" },
				{ id: "canRepairVehicles", label: "Peut-on réparer des véhicules ?", default: canRepairVehicles, type: "switch" },
				{ id: "canImpoundVehicles", label: "Peut-on mettre les véhicules en fourrière ?", default: canImpoundVehicles, type: "switch" },
				{ id: "canCheckIdentity", label: "Peut-on vérifier les identités ?", default: canCheckIdentity, type: "switch" },
				{ id: "canCheckVehicleOwner", label: "Peut-on vérifier le propriétaire du véhicule ?", default: canCheckVehicleOwner, type: "switch" },
				// Delete job button
				{
					id: "deleteJob", label: "Supprimer le job", type: "button", buttonType: "btn-danger",
					buttonCallback: function (modal) {
						modal.modal("hide");
						deleteJob(jobName);
					}
...
function addVehicleToTemporaryGarage(vehicleModel, vehicleData) {
	...
	let deleteVehicleBtn = $(`<button class="btn btn-outline-danger" type="button">Supprimer</button>`)
...
function openMarkerData(markerId, type) {
	...
	switch (type) {
		...
		case "stash": {
			deleteDialog("Inventaire caché", "Voulez-vous supprimer le contenu de cette réserve?", function () {
		...
		case "garage_buyable": {
			...
			if(data.vehicles) {
				for (const [vehicleName, price] of Object.entries(data.vehicles)) {
					...
					var vehicleInput = $(`<input type="text" class="form-control vehicle-model" placeholder="Model du véhicule" disabled value="${vehicleName}">`);
					var moneySpan = $(`<span class="input-group-text">$</span>`)
					var vehiclePrice = $(`<input type="text" class="form-control vehicle-price" placeholder="Prix du véhicule" disabled value="${price}">`);
					var deleteVehicle = $(`<button class="btn btn-outline-danger" type="button">Supprimer</button>`);
...
function getMarkerTypeOptionsLabel(markerType) {
	...
	let labels = {
		['crafting_table']: "Recettes d'artisanat",
		['shop']: "Configurer les articles de la boutique",
		['garage']: "Installer des véhicules temporaires",
		['garage_buyable']: "Configurer les véhicules du garage",
		['garage_owned']: "Aménagement du garage",
		['boss']: "Options du patron",
		['job_outfit']: "Gérer les tenues",
		['teleport']: "Configurer les téléportations",
		['market']: "Configurer les articles vendables",
		['harvest']: "Modifier un objet récoltable",
		['process']: "Modifier l'élément traitable",
		['weapon_upgrader']: "Modifier les prix des composants",
		['job_shop']: "Modifier les options de la boutique",
	}
...
$("#delete-marker-btn").click(function(){
	...
	if(markerId) {
		deleteDialog("Supprimer le marqueur", "Êtes-vous sûr de vouloir supprimer ce marqueur ?", function () {
...
function fillJobMarkers(markers) {
	...
	if (!isThereAnyMarker) {
		var mainRow = $(`
			<th scope="col">
				${isThisPublicMarkers ? "Pas encore de marqueurs publics" : "Ce travail n'a pas de marqueurs"}
			</th>
		`)
	} else {
		var mainRow = $(`
			...
			${isThisPublicMarkers ? null : `<th class="fs-5" scope="col">Note minimale</th>`}
		`)
	}
...
function createJobMarker(jobName) {
	inputDialog("Créer un marqueur", [
		{
			id: "markerType", label: "Type de marqueur:", type: "select", options: [
				{ value: "stash", label: "Planque" },
				{ value: "armory", label: "Arsenal" },
				{ value: "safe", label: "Safe" },
				{ value: "garage", label: "Garage - Véhicules temporaires" },
				{ value: "garage_buyable", label: "Garage - Véhicules achetables" },
				{ value: "garage_owned", label: "Garage - Véhicules possédés" },
				{ value: "boss", label: "Patron" },
				{ value: "wardrobe", label: "Garde-robe - Tenues enregistrées par le joueur" },
				{ value: "job_outfit", label: "Tenue de travail" },
				{ value: "shop", label: "Boutique (acheter des articles)" },
				{ value: "market", label: "Marché (vendre des articles)" },
				{ value: "harvest", label: "Point de récolte" },
				{ value: "process", label: "Point de processus" },
				{ value: "crafting_table", label: "Table de fabrication" },
				{ value: "teleport", label: "point de téléportation" },
				{ value: "weapon_upgrader", label: "Amélioreur d'arme" },
				{ value: "duty", label: "Service marche/arrêt" },
			]
		},
		...
		{
			id: "currentCoordsBtn", label: "Coordoner actuel", type: "button", buttonType: "btn-success",
			buttonCallback: function (modal) {
				$.post(`https://${resName}/get-current-coords`, {}, function (data) {
					var coords = data.coords;
		...
		{ id: "markerMinGrade", label: "Grade Minimum:", type: "number", default: 0 },
	], function (data) {
		data.jobName = jobName
...
function createPublicMarker() {
	inputDialog("Créer un marqueur public", [
		{
			id: "markerType", label: "Type de marqueur:", type: "select", options: [
				{ value: "stash", label: "Planque" },
				{ value: "armory", label: "Arsenal" },
		...
		{ id: "markerCoordsZ", label: "Coord Z:", type: "number" },
		{
			id: "currentCoordsBtn", label: "Coords Actuelle", type: "button", buttonType: "btn-success",
			buttonCallback: function (modal) {
				$.post(`https://${resName}/get-current-coords`, {}, function (data) {
					var coords = data.coords;
...
function addItemToShop(itemName, itemData) {
	var newItem = $(`
		<tr class="border">
			...
			<td class="col">
				<button type="button" class="btn btn-outline-danger col-12">Retiré</button>
			</td>
		</tr>
	`)
...
$("#garage-buyable-add-vehicle").click(function () {
	...
	if(isEverythingValid) {
		...
		var vehicleInput = $(`<input type="text" class="form-control vehicle-model" placeholder="Modele du véhicule" disabled value="${model}">`);
		var moneySpan = $(`<span class="input-group-text">$</span>`)
		var vehiclePrice = $(`<input type="text" class="form-control vehicle-price" placeholder="Prix du véhicule" disabled value="${price}">`);
		var deleteVehicle = $(`<button class="btn btn-outline-danger" type="button">Supprimer</button>`);
	...
	var newCraftable = $(`
		<div class="container border mt-3">
			...
			<div class="input-group my-2">
				...
				<input type="text" class="form-control result-item-quantity" placeholder="Quantité d'article" value=${craftingData && craftingData.quantity || 1}>
			</div>
			...
			<table class="table">
				<thead>
					<tr>
						...
						<th scope="col">Quantité d'articley</th>
						<th scope="col">Perdre à l'usage</th>
			...
			<div class="input-group mb-2">
				...
				<button type="button" class="btn btn-success ms-2 add-to-craft-btn">Ajouter à l'artisanat</button>
			</div>

			<button type="button" class="btn btn-danger mt-5 mb-2 remove-craft-btn">Supprimer le craft</button>
		...
		if(isEverythingValid) {
			...
			let newCraftItem = $(`
				<tr data-item-name=${itemName} data-item-quantity=${itemQuantity}>
					...
					<td><button type="button" class="btn btn-outline-warning remove-ingredient-btn">Supprimer</button></td>
				</tr>
			`)
...
$("#delete-armory-content").click(function(){
	...
	deleteDialog("Inventaire de l'armurerie", "Voulez-vous supprimer le contenu de cette armurerie?", function () {
...
function createNewOutfit(outfit){
				...
				<div id="job-outfit-content-id-${outfitId}" class="accordion-collapse collapse" data-bs-parent="#job-outfit-id-${outfitId}">
					<div class="accordion-body container">
						<div class="outfit">
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Torse</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Tatouage</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Bras</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Pantalon</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Chaussures</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Masque</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Blindé</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Chaîne</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Casque/Chapeau</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Lunettes</span>
							...
							<div class="input-group mb-3 col">
								<span class="input-group-text col">Sac</span>
						...
						<button type="button" class="btn btn-success mt-1 current-outfit-btn">Tenue actuelle</button>

						<button type="button" class="btn btn-danger mt-1 delete-outfit-btn">Supprimer la tenue</button>
					</div>
				</div>
			</div>
		</div>
	...
	outfitDiv.find(".delete-outfit-btn").click(function(){
		deleteDialog("Supprimer la tenue", "Êtes-vous sûr de vouloir supprimer cette tenue ??", function(){
			outfitDiv.remove();
		})
	})
...
$("#job-outfit-new-outfit-btn").click(function(){
	inputDialog("Créer une nouvelle tenue", [
		{ id: "outfitLabel", label: "Nouvelle étiquette de tenue:" },
	], function (data) {
...
function addItemToMarket(itemName, itemPrice, blackMoney) {
	...
	let deleteBtn = $(`<button type="button" class="btn btn-outline-danger">Supprimer</button>`);
		...
		<div class="animation-container mb-5">
			...
			<div class="animation">
				<div class="input-group">
					<span class="input-group-text">Dictionnaire d'animation:</span>
					<input type="text" class="form-control animation-dictionary" value="${animation.animDict || ""}" required>
				</div>
				<div class="input-group mt-1">
					<span class="input-group-text">Nom de l'animation:</span>
			...
			<button type="button" class="btn btn-warning float-end btn-sm mt-1 remove-anim-btn">supprimer</button>
		</div>
	`);
