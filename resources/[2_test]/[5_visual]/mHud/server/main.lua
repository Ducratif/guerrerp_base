...
ESX.RegisterServerCallback('mHud:getPlayerVipMoney', function(source, cb)
    print("[SERVER] Register call back")
    local xPlayer = ESX.GetPlayerFromId(source)
    local vipMoney = MySQL.scalar.await('SELECT coins FROM users WHERE identifier = ?', {xPlayer.identifier})
    cb(vipMoney or 0)
end)

-- Exemple fonction pour ajouter/soustraire de l'argent VIP
RegisterNetEvent("mHud:updateVipMoney")
AddEventHandler("mHud:updateVipMoney", function(amount)
    print("[SERVER]Event Handler")
    local xPlayer = ESX.GetPlayerFromId(source)
    local currentMoney = MySQL.scalar.await('SELECT coins FROM users WHERE identifier = ?', {xPlayer.identifier})
    local newAmount = (currentMoney or 0) + amount

    MySQL.update.await('UPDATE users SET coins = ? WHERE identifier = ?', {newAmount, xPlayer.identifier})
    TriggerClientEvent("mHud:VipMoneyAmountChanged", source, newAmount)
end)
