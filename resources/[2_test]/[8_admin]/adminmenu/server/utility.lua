...
if src then
                ...
                if not identifier then
                    print("^1[AdminMenu] ERREUR: Aucun identifier trouvé pour src: " .. tostring(src) .. "^0")
                    TriggerClientEvent('codem-adminmenu:notfication', src, 'Aucun identifiant trouvé.', 'notification')
                    return
                end
        
                local data = playerAdminData[identifier]
                if not data then
                    -- Vérifier si la table est vide (aucun admin enregistré)
                    local allAdmins = ExecuteSql("SELECT * FROM codem_adminmenu")
                    if #allAdmins == 0 then
                        -- C'est le premier admin ! On l'ajoute
                        local defaultData = {
                            identifier = identifier,
                            permissiondata = json.encode({}),
                            historydata = json.encode({}),
                            bandata = json.encode({}),
                            profiledata = json.encode({})
                        }
                
                        ExecuteSql("INSERT INTO codem_adminmenu (identifier, permissiondata, historydata, bandata, profiledata) VALUES (@identifier, @permissiondata, @historydata, @bandata, @profiledata)", defaultData)
                
                        print("^2[AdminMenu] Le premier administrateur a été enregistré automatiquement :^0 " .. identifier)
                        ...
                        -- Il y a déjà un ou plusieurs admins => on bloque
                        TriggerClientEvent('codem-adminmenu:notfication', src,
                            'Tu n\'es pas autorisé à accéder au menu admin.',
                            'notification')
                        return
                    end
                end
                ...
                TriggerClientEvent('codem-adminmenu:client:updateOnlinePlayersData', -1, identifier,
                    onlinePlayersData[identifier])
                TriggerClientEvent('codem-adminmenu:notfication', src, 'Admin Menu Loaded..', 'notification')
                saveDataOnline(src)
            end
        end, true, {
