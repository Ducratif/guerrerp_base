        ...
        if distance < 1.25 then
            if Config.Interact.Enabled then
                ...
                if Config.PhotoPrice > 0 then
                    ESX.ShowHelpNotification(Config.Texts["interact_paid_photo"])
                else
                    ESX.ShowHelpNotification(Config.Texts["interact_free_photo"])
                end
            end
...
AddEventHandler("vms_document:showMyDocument", function(type)
    ESX.TriggerServerCallback("vms_document:getImage", function(url)
        if url then
