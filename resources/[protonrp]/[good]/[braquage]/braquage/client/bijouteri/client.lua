...
local menuDescByHouseState = {
    ...
    [false] = { title = "Repassez plus tard", desc = "Cette bijouterie a été récemment vandalisée par un Timoté, repassez plus tard..", style = {} },
}
...
Citizen.CreateThread(function()
    ...
    RMenu.Add("ducratif_bijouteri", 'robb', RageUI.CreateMenu("Braquage","Braquer la bijouterie"))
    RMenu:Get("ducratif_bijouteri", 'robb'):SetRectangleBanner(0, 0, 0, 240); --Menu noir
    initMenuLoope()
            ...
            for index,house in pairs(bijouterie.houses) do
                ...
                if thisHouseDistance <= 15 then
                    ...
                    if thisHouseDistance <= 1 then
                        closeToHouse = index
                        if not RageUI.Visible(RMenu:Get("ducratif_bijouteri",'robb')) then
                            RageUI.Text({message = "Appuyez sur ~o~[E]~s~ pour braqué",time_display = 1})
                        end
                        if IsControlJustPressed(0, 51) then
                            if IsPedArmed(PlayerPedId(), 4) then
                                ...
                                --ESX.ShowAdvancedNotification("Action impossible","~r~Braquage","Tu essaye de braqué sans armes ? Ta crue tu est Superman PTDRR bouge de la ptit joueur !","CHAR_DEVIN",9)
                            exports.ox_lib:notify({
                                title = 'Braquage',
                                description = ("Tu essaye de braqué sans armes ? Ta crue tu est Superman PTDRR bouge de la ptit joueur !"),
                                type = 'error',
                                
                            })
                            end
                        end
                    elseif thisHouseDistance > 1 and RageUI.Visible(RMenu:Get("ducratif_bijouteri",'robb')) then
                        RageUI.Visible(RMenu:Get("ducratif_bijouteri",'robb'), false)
                    end
                else
                ...
                if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 150.0 then
                    ...
                    if GetDistanceBetweenCoords(pos,resellerPos.vector,false) <= 3.0 then
                        ...
                        if IsControlJustPressed(0, 51) then
                            ...
                            TriggerServerEvent("ducratif_bijouterie:reward", finalReward)
                            TriggerServerEvent("ducratif_bijouterie:money", finalReward)
                            ...
                            exports.ox_lib:notify({
                                title = 'Bijouterie',
                                description = ("Félicitations tu as recus "..finalReward.."$ grâce aux objects que tu as volé! Je te recontacterai"),
                                type = 'success',
...
function terminatee()
    ...
    if #robbItems ~= 0 then
        ...
        exports.ox_lib:notify({
            title = 'Bijouterie',
            description = ("Bravo pour ton vol, tu as "..#robbItems.." objects sur toi. Rends toi devant le reseller pour les revendres!"),
            type = 'success',
    ...
    Citizen.SetTimeout((1000*60)*bijouterie.houses[currentRobbingData.houseIndex].timeToGoToReseller, function()
        if not itemsSold then
            ...
            exports.ox_lib:notify({
                title = 'Action impossible',
                description = ("Tu fais déplacer un reseller pour au final pas te pointer. Tkt mec il va venir pour te buter !"),
                type = 'error',
...
function initMenuLoope()
    Citizen.CreateThread(function()
        while true do
            RageUI.IsVisible(RMenu:Get("ducratif_bijouteri",'robb'),true,true,true,function()
                RageUI.ButtonWithStyle(menuDescByHouseState[housesData[closeToHouse].state].title,menuDescByHouseState[housesData[closeToHouse].state].desc, menuDescByHouseState[housesData[closeToHouse].state].style, housesData[closeToHouse].state, function(Hovered,Active,Selected)
                    if Selected then
                        ...
                        if not thisHouseAlreadyRobbed then
                            exited = false
                            Citizen.Wait(1000)
                            TriggerServerEvent("ducratif_bijouterie:houseRobbed", closeToHouse)
                            isRobbing = true
            ...
            if not copsCalleds then
                if currentRobbingData.data.copsCalledAfter <= 0 then
                    ...
                    exports.ox_lib:notify({
                        ...
                        description = ("Un témoin oculaire vous a apercu et a contacté les forces de l'ordre, la police a donc été prévenue et connait votre localisation. EN GROS FUIT CHAKAL !"),
                        type = 'error',

                    })
                    TriggerServerEvent("ducratif_bijouterie:callThePolice", currentRobbingData.houseIndex)
                else
...
function alterMenuVisibilitys_()
    RageUI.Visible(RMenu:Get("ducratif_bijouteri",'robb'), not RageUI.Visible(RMenu:Get("ducratif_bijouteri",'robb')))
end
...
RegisterNetEvent("ducratif_braquage:initializePoliceBlipx")
AddEventHandler("ducratif_braquage:initializePoliceBlipx", function(houseIndex,duration)
    ...
    exports.ox_lib:notify({
        title = 'Central de police',
        description = ("Un civil a appellé la police."),
        type = 'error',
