...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(Config.TimeToWait)
        --exports.esx_xp.
	ESXP_Add(Config.XpToWin)
    end
end)
Citizen.CreateThread(function()
	while true do 
		Citizen.Wait(0)
		local playerranks = ESXP_GetRank()
		if playerranks <= 4 then
			DisableControlAction(0, 142, true)
			DisablePlayerFiring(GetPlayerPed(-1), true)
			if IsDisabledControlJustReleased(0, 142) then
				ESX.ShowHelpNotification('Vous ne pouvez pas ~o~frapper/tirer~s~ en dessous du niveau 5 et demi.!')
			end
		else
