                ...
                for _, player in pairs(GetActivePlayers()) do
                    ...
                    -- if ped ~= PlayerPedId() then
                        -- if #(GetEntityCoords(plyPed, false) - GetEntityCoords(ped, false)) < 5000.0 then
                            local playerRank = ''
                            if YveltHelper.staffsInfos['id:'..GetPlayerServerId(player)] then
                                playerRank = YveltHelper.staffsInfos['id:'..GetPlayerServerId(player)].rankLabel
                            end
                            gamerTags[player] = CreateFakeMpGamerTag(ped, ('[ID : %s | UID : %s] %s %s'):format(GetPlayerServerId(player), UID:getUIDfromID(GetPlayerServerId(player)), GetPlayerName(player), playerRank), false, false, '', 0)
                            ...
                            if YveltHelper.staffsActive['id:'..GetPlayerServerId(player)] then
                            ...
                            if NetworkIsPlayerTalking(player) then
                                ...
                                if playerRank then
                                    if YveltHelper.staffsActive['id:'..GetPlayerServerId(player)] then
                                        ...
                                        SetMpGamerTagColour(gamerTags[player], 4, YveltHelper.colorsNames[YveltHelper.staffsInfos['id:'..GetPlayerServerId(player)].rankColor])
                                        SetMpGamerTagColour(gamerTags[player], 0, YveltHelper.colorsNames[YveltHelper.staffsInfos['id:'..GetPlayerServerId(player)].rankColor])
                                    else
                                        SetMpGamerTagHealthBarColour(gamerTags[player], 0)
