...
if ESX.GetConfig().OxInventory then
    AddEventHandler('onServerResourceStart', function(resourceName)
		--print(exports.ox_inventory)
        if resourceName == 'ox_inventory' or resourceName == GetCurrentResourceName() then
            -- Récupérer les stashes depuis la base de données
            local stashes = MySQL.query.await('SELECT * FROM addon_inventory')

            for i=1, #stashes do
                local stash = stashes[i]
                local jobStash = stash.name:find('society') and string.sub(stash.name, 9) or nil

                -- Ajoute le stash à ox_inventory avec les paramètres nécessaires
                -- Utilisation de AddJobStash pour enregistrer un stash lié à un job
                if jobStash then
                    --exports.ox_inventory:AddJobStash(stash.name, stash.label, 100, 200000, stash.shared == 0 and true or false, jobStash)
                	exports['ox_inventory']:AddJobStash("job_stash_name", "Stash du métier", 100, 200000, 0, "police")
				--local jobStash = exports['ox_inventory']:AddJobStash("job_stash_name", "Stash du métier", 100, 200000, 0, "police")
				--print("Stash créé pour le job 'police' avec le nom : " .. jobStash.name)

				else
                    -- Si ce n'est pas un job, on peut l'ajouter comme stash personnel
                    --exports.ox_inventory:AddPersonalStash(stash.name, stash.label, 100, 200000)
						exports['ox_inventory']:AddPersonalStash("stash_name", "Mon Stash", 100, 200000)
					--local stash = exports['ox_inventory']:AddPersonalStash("stash_name", "Mon Stash", 100, 200000)
					--print("Stash créé avec le nom : " .. stash.name)
                end
            end
        end
    end)
