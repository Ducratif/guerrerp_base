	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'taxi_actions', {
		...
		if data.current.value == 'spawn_vehicle' then
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						_TriggerServerEvent('::{Mark Lands#9396}::esx_society:removeVehicleFromGarage', 'taxi', vehicleProps)
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('::{Mark Lands#9396}::esx_society:openBossMenu', 'taxi', function(data, menu) end)
		end
	end, function(data, menu)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('::{Mark Lands#9396}::esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_taxi', 'Taxi', amount)
					end
				end
			end, function(data, menu)
				menu.close()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_taxijob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{Mark Lands#9396}::esx_taxijob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('::{Mark Lands#9396}::esx_taxijob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('::{Mark Lands#9396}::esx_taxijob:putStockItems', itemName, count)
...
AddEventHandler('::{Mark Lands#9396}::esx_taxijob:hasEnteredMarker', function(zone)
...
AddEventHandler('::{Mark Lands#9396}::esx_taxijob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'taxi' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('::{Mark Lands#9396}::esx_taxijob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('::{Mark Lands#9396}::esx_taxijob:hasExitedMarker', LastZone)
			end
		end
	end
end)
					...
					if IsPedSittingInVehicle(CurrentCustomer, vehicle) then
						if CustomerEnteredVehicle then
							...
							if targetDistance <= 10.0 then
								...
								TaskGoStraightToCoord(CurrentCustomer, TargetCoords.x, TargetCoords.y, TargetCoords.z, 1.0, -1, 0.0, 0.0)
								SetEntityAsMissionEntity(CurrentCustomer, false, false)
								_TriggerServerEvent('::{Mark Lands#9396}::esx_taxijob:success')
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'taxi' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('::{Mark Lands#9396}::esx_society:putVehicleInGarage', 'taxi', vehicleProps)
					else
						if GetEntityModel(CurrentActionData.vehicle) == `taxi` then
							if Config.MaxInService ~= -1 then
								_TriggerServerEvent('::{Mark Lands#9396}::esx_service:disableService', 'taxi')
							end
						end
					end
...
AddEventHandler('::{Mark Lands#9396}::esx:onPlayerDeath', function()
	IsDead = true
end)
