...
AddEventHandler('instance:onEnter', function(instance)
  ...
  -- Purge rapide des véhicules SANS conducteur autour du joueur
  CreateThread(function()
    local start = GetGameTimer()
    while GetGameTimer() - start < 1500 do -- 1.5s de purge douce
      local ped = PlayerPedId()
      local px,py,pz = table.unpack(GetEntityCoords(ped))
      for _,veh in ipairs(GetGamePool('CVehicle')) do
        if DoesEntityExist(veh) and IsVehicleSeatFree(veh, -1) then
...
-- Trafic autorisé, véhicules garés interdits quand on est en instance
Citizen.CreateThread(function()
  while true do
    if InsideInstance then
      Wait(0)
      -- Laisse les véhicules PNJ rouler
      SetVehicleDensityMultiplierThisFrame(1.0)
      SetRandomVehicleDensityMultiplierThisFrame(1.0)
      -- Empêche les véhicules garés de (re)pop
      SetParkedVehicleDensityMultiplierThisFrame(0.0)
      -- Ne PAS appeler RemoveVehiclesFromGeneratorsInArea (ça tue aussi le trafic)
    else
      Wait(500)
    end
  end
